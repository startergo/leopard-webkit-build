...
template<> EncodedJSValue JSC_HOST_CALL JSAnimationPlaybackEventConstructor::construct(JSGlobalObject* lexicalGlobalObject, CallFrame* callFrame)
{
    ...
    auto eventInitDict = convert<IDLDictionary<AnimationPlaybackEventInit>>(*lexicalGlobalObject, argument1.value());
    RETURN_IF_EXCEPTION(throwScope, encodedJSValue());
    auto object = AnimationPlaybackEvent::create(WTFMove(type), WTFMove(eventInitDict));
    auto jsValue = toJSNewlyCreated<IDLInterface<AnimationPlaybackEvent>>(*lexicalGlobalObject, *castedThis->globalObject(), WTFMove(object));
...
template<> void JSAnimationPlaybackEventConstructor::initializeProperties(VM& vm, JSDOMGlobalObject& globalObject)
{
    ...
    putDirect(vm, vm.propertyNames->name, jsNontrivialString(vm, "AnimationPlaybackEvent"_s), JSC::PropertyAttribute::ReadOnly | JSC::PropertyAttribute::DontEnum);
...
const ClassInfo JSAnimationPlaybackEventPrototype::s_info = { "AnimationPlaybackEvent", &Base::s_info, nullptr, nullptr, CREATE_METHOD_TABLE(JSAnimationPlaybackEventPrototype) };
...
JSC::IsoSubspace* JSAnimationPlaybackEvent::subspaceForImpl(JSC::VM& vm)
{
    auto& clientData = *static_cast<JSVMClientData*>(vm.clientData);
    auto& spaces = clientData.subspaces();
    if (auto* space = spaces.m_subspaceForAnimationPlaybackEvent.get())
        return space;
    static_assert(std::is_base_of_v<JSC::JSDestructibleObject, JSAnimationPlaybackEvent> || !JSAnimationPlaybackEvent::needsDestruction);
    if constexpr (std::is_base_of_v<JSC::JSDestructibleObject, JSAnimationPlaybackEvent>)
        spaces.m_subspaceForAnimationPlaybackEvent = makeUnique<IsoSubspace> ISO_SUBSPACE_INIT(vm.heap, vm.destructibleObjectHeapCellType.get(), JSAnimationPlaybackEvent);
    else
        spaces.m_subspaceForAnimationPlaybackEvent = makeUnique<IsoSubspace> ISO_SUBSPACE_INIT(vm.heap, vm.cellHeapCellType.get(), JSAnimationPlaybackEvent);
    auto* space = spaces.m_subspaceForAnimationPlaybackEvent.get();
IGNORE_WARNINGS_BEGIN("unreachable-code")
IGNORE_WARNINGS_BEGIN("tautological-compare")
    if (&JSAnimationPlaybackEvent::visitOutputConstraints != &JSC::JSCell::visitOutputConstraints)
