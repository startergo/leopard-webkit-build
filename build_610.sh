...
ICU_SRC="$DOWNLOADS_DIR/icu62-src/icu/icu4c"  # [leopard] ICU 62.2 (release-62-2 submodule) to match WebKit 610 bundled headers
...
phase0_prerequisites() {
    ...
    SDK_OVERLAY_SRC="$PROJECT_ROOT/patches-610/overlay-sdk"
    if [ ! -f "$OVERLAY_DIR/TargetConditionals_compat.h" ] || [ ! -f "$OVERLAY_DIR/sdk_stubs.mm" ]; then
        if [ -d "$SDK_OVERLAY_SRC" ]; then
            info "Installing SDK-compat overlay from tracked patches-610/overlay-sdk ..."
            mkdir -p "$OVERLAY_DIR"
            cp -R "$SDK_OVERLAY_SRC/." "$OVERLAY_DIR/"
        else
            err "SDK-compat overlay source missing: $SDK_OVERLAY_SRC"
            exit 1
        fi
    fi
...
phase4_cmake() {
    ...
    cmake -G Ninja \
        ...
        -DENABLE_WEBGL=ON -DENABLE_WEBGL2=OFF \
