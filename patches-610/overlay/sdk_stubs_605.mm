...
static int xpc_type_bool_val = 0;
void *XPC_TYPE_BOOL = &xpc_type_bool_val;  // [leopard] match XPC_TYPE_ERROR pattern
...
@implementation NSColor (LeopardCGColor)
- (CGColorRef)CGColor {
    NSColor *rgb = [self colorUsingColorSpaceName:NSCalibratedRGBColorSpace];
    if (!rgb) rgb = self;
    CGFloat r = 0, g = 0, b = 0, a = 1;
    @try { [rgb getRed:&r green:&g blue:&b alpha:&a]; } @catch (...) {}
    CGColorSpaceRef cs = CGColorSpaceCreateDeviceRGB();
    CGFloat comps[4] = { r, g, b, a };
    CGColorRef cg = CGColorCreate(cs, comps);
    CGColorSpaceRelease(cs);
    return (CGColorRef)CFAutorelease(cg);
}
@end


/* [leopard-webkit-build] NSGraphicsContext.CGContext (10.10+) -> -graphicsPort on 10.6.
   Satisfies dot-syntax .CGContext accesses in ColorMac/DragImageCocoa/RenderThemeMac/
   ThemeMac/LocalCurrentGraphicsContextMac. Declared in Overlay605Supplement.h. */
@implementation NSGraphicsContext (LeopardCGContext)
- (CGContextRef)CGContext { return (CGContextRef)[self graphicsPort]; }
@end



/* [leopard] CGPathCreateMutableCopyByTransformingPath (10.7+) — copy a path applying a transform. */
extern "C" {
static void leopardPathApplier_t(void *info, const CGPathElement *element)
{
    CGMutablePathRef dst = (CGMutablePathRef)((void**)info)[0];
    CGAffineTransform *t = (CGAffineTransform *)((void**)info)[1];
    CGPoint *pts = element->points;
    switch (element->type) {
    case kCGPathElementMoveToPoint: CGPathMoveToPoint(dst, t, pts[0].x, pts[0].y); break;
    case kCGPathElementAddLineToPoint: CGPathAddLineToPoint(dst, t, pts[0].x, pts[0].y); break;
    case kCGPathElementAddQuadCurveToPoint: CGPathAddQuadCurveToPoint(dst, t, pts[0].x, pts[0].y, pts[1].x, pts[1].y); break;
    case kCGPathElementAddCurveToPoint: CGPathAddCurveToPoint(dst, t, pts[0].x, pts[0].y, pts[1].x, pts[1].y, pts[2].x, pts[2].y); break;
    case kCGPathElementCloseSubpath: CGPathCloseSubpath(dst); break;
    }
}
CGMutablePathRef CGPathCreateMutableCopyByTransformingPath(CGPathRef path, const CGAffineTransform *transform)
{
    ...
    void *info[2] = { dst, (void *)transform };
    CGPathApply(path, info, leopardPathApplier_t);
    return dst;
}
} /* extern "C" */

/* [leopard] dyld_get_program_sdk_version() is a 10.7+ dyld API absent on 10.6. Provide a real
   (non-inline) definition here so WTF/JSC references resolve at link time. Reports 10.6. */
